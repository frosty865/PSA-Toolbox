...
console.log(`Connecting to RUNTIME database: ${connectionInfo}`);
...
async function main() {
  ...
  if (!migrationFile) {
    console.error('Usage: node scripts/run_runtime_migration.js <migration_file.sql>');
    process.exit(1);
  }
  ...
  if (!connectionString) {
    console.error('RUNTIME_DATABASE_URL not found in environment');
    process.exit(1);
  }
  
  try {
    console.log(`Running RUNTIME migration: ${filePath}`);
