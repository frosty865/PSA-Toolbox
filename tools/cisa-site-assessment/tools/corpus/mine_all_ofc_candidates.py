#!/usr/bin/env python3
"""
CORPUS: Mine OFC Candidates from All Documents in Active Source Set

Mines OFC candidates from all documents in the active source set.
...
from tools.mine_ofc_candidates_from_chunks import mine_candidates

def mine_all_ofc_candidates(authority_scope: str = "BASELINE_AUTHORITY", max_candidates_per_doc: int = 250):
    """
    Mine OFC candidates from all documents in the active source set.
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Mine OFC candidates from all documents in active source set')
    ...
    try:
        result = mine_all_ofc_candidates(
        ...
        if result['status'] == 'completed':
            print(f"\n✅ OFC candidate mining complete: {result['total_candidates']} total candidates from {result['documents_processed']} documents")
