...
function isGenericDescription(description: string, subsectorName: string, sectorName: string) {
...
export default function SectorsPage() {
  ...
  const [subsectorsMap, setSubsectorsMap] = useState<Record<string, any[]>>({})
  ...
  const loadSubsectorsForSector = useCallback(async (sector: any, showLoading = true) => {
    ...
    try {
      ...
      const sectorName = sector.sector_name || sector.name || 'Unknown'
      
      const subsResponse = await fetch(`/api/reference/subsectors?sectorId=${encodeURIComponent(sectorId)}`, { 
        cache: 'no-store',
        headers: {
      ...
      if (!subsResponse.ok) {
        ...
        console.error(`[SectorsPage] API error for ${sectorName}:`, subsResponse.status, errorText)
  ...
  useEffect(() => {
    ...
    const loadSectors = async () => {
      try {
        ...
        setLoading(true)
        const response = await fetch('/api/reference/sectors', { cache: 'no-store' })
        if (!response.ok) {
        ...
        activeSectors.forEach((sector: any) => {
  ...
  const filteredSectors = useMemo(() => {
    ...
    return sectors.filter(sector => {
      ...
      return subsectors.some((sub: any) => {
