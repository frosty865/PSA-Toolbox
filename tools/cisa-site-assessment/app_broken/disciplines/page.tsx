...
export default function DisciplinesPage() {
  ...
  useEffect(() => {
    const loadDisciplines = async () => {
      try {
        ...
        const response = await fetch('/api/reference/disciplines?active=true', { cache: 'no-store' })
        
        if (!response.ok) {
          const errorText = await response.text()
          console.error('[DisciplinesPage] API response not OK:', response.status, errorText)
          throw new Error(`Failed to load disciplines: ${response.status} ${errorText}`)
        }
        ...
        // Debug: Log subtypes for all disciplines
        console.log('[DisciplinesPage] Raw disciplines received:', rawDisciplines.length);
        rawDisciplines.forEach((d: any, idx: number) => {
          const subtypes = d.discipline_subtypes || [];
          console.log(`[DisciplinesPage] Discipline ${idx + 1}: "${d.name}" - subtypes:`, {
            count: subtypes.length,
            subtypes: subtypes.map((st: any) => ({ id: st.id, name: st.name, is_active: st.is_active }))
          });
        });
        ...
        const uniqueDisciplines = (rawDisciplines as any[]).filter((d: any) => {
          if (!d.id) {
        ...
        const finalDisciplines = uniqueDisciplines.filter((d: any) => {
        ...
        const disciplinesData: Discipline[] = finalDisciplines
          ...
          .map((d: any) => {
            ...
            if (subtypes.length > 0) {
              console.log(`[DisciplinesPage] Preserving ${subtypes.length} subtypes for "${d.name}"`);
            }
            return {
              ...d,
        ...
        console.log('[DisciplinesPage] Final disciplines data:', {
          count: disciplinesData.length,
          totalSubtypes: disciplinesData.reduce((sum: number, d: Discipline) => sum + (d.discipline_subtypes?.length || 0), 0)
        });
  ...
  const loadSubtypeDetails = async (subtype: DisciplineSubtype) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to load subtype details: ${response.status}`)
      }
  ...
  const hasArrayContent = (field: any) => {
    if (!field) {
      return false
    }
            ...
            <div key={category}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-md)' }}>
                {categoryDisciplines.map(discipline => {
                  ...
                  // Debug logging
                  console.log(`[DisciplinesPage] Rendering discipline "${disciplineName}":`, {
                    disciplineId,
                    isExpanded,
                    totalSubtypes: subtypes.length,
                    subtypes: subtypes.map((st: any) => ({ 
                      id: st.id, 
                      name: st.name, 
                      is_active: st.is_active,
                      discipline_id: st.discipline_id 
                    }))
                  });
                  
                  // Filter active subtypes (is_active can be true, null, or undefined - all should be shown)
                  const activeSubtypes = subtypes.filter((st: any) => {
                    // Show if is_active is true, null, or undefined (only hide if explicitly false)
                    const isActive = st.is_active !== false;
                    if (!isActive) {
                      console.log(`[DisciplinesPage] Filtering out inactive subtype:`, st);
                    }
                    return isActive;
                  });
                          ...
                          {subtypeCount > 0 ? (
                            <div>
                              ...
                              <div style={{
                                ...
                                {activeSubtypes.map((subtype: any) => {
                                  console.log(`[DisciplinesPage] Rendering subtype card:`, subtype.name, subtype.id);
                                  if (!subtype.id) {
