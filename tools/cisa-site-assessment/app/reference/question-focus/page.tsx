...
export default function QuestionFocusPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const disciplinesData = await fetchJson(
          '/api/reference/disciplines?active=true',
          { cache: 'no-store' },
          parseDisciplinesPayload
        );
        if (!disciplinesData.success) {
          throw new Error(disciplinesData.error || 'Failed to load disciplines');
        }
        const allDisciplines = disciplinesData.disciplines.filter((d) => d.is_active !== false);
  ...
  const pagesByDisciplineCode = pages.reduce((acc, page) => {
    ...
    return acc;
  }, {} as Record<string, Record<string, QuestionFocusPageDto>>);
  ...
  const getQuestionFocusPage = (disciplineCode: string, subtype: DisciplineSubtype): QuestionFocusPageDto | null => {
    const normalizedDisciplineCode = disciplineCode.toUpperCase();
    const normalizedSubtypeCode = (subtype.code || '').toUpperCase();
  ...
  const filteredDisciplines = searchTerm
    ? disciplines.filter(d => {
        ...
        const matchesSubtype = (d.discipline_subtypes || []).some((st) =>
  ...
  const totalSubtypes = disciplines.reduce((sum, d) => {
    const activeSubtypes = (d.discipline_subtypes || []).filter((st) => st.is_active !== false);
  ...
  return (
    <section className="section active">
      <div className="section-header">
        <h1 className="section-title">Question Focus Pages</h1>
        <p style={{
          ...
          These pages describe the focus and intent of baseline questions for each discipline subtype.
        </p>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-md)' }}>
          {filteredDisciplines
            ...
            .map((discipline) => {
              ...
              const allSubtypes = (discipline.discipline_subtypes || []).filter((st) => st.is_active !== false);
