...
/**
 * Queue REGISTERED CORPUS documents for processing.
 * Marks them PROCESSING and enqueues into corpus_reprocess_queue.
...
export async function POST() {
  ...
  try {
    ...
    // Queue only traceable docs (source_registry_id IS NOT NULL)
    const select = await client.query(`
      SELECT id
      FROM public.corpus_documents
      WHERE processing_status = 'REGISTERED'
        AND source_registry_id IS NOT NULL
      ORDER BY COALESCE(created_at, processed_at) DESC NULLS LAST, COALESCE(inferred_title, file_stem, original_filename) ASC
      LIMIT 500
      FOR UPDATE
    `);
    ...
    if (ids.length === 0) {
      // Report how many are unqueueable due to missing source_registry_id
    ...
    // Also report how many REGISTERED docs cannot be queued due to missing source_registry_id
    const skipped = await client.query(`
      SELECT COUNT(*)::int AS count
      FROM public.corpus_documents
      WHERE processing_status = 'REGISTERED'
