# CREATE NEW SOUTH FLORIDA JSON - Valid Miami address
...
Write-Host "=== CREATING SOUTH FLORIDA JSON ===" -ForegroundColor Green
...
# REALISTIC ASSESSMENT RESPONSES
$realisticResponses = @{
    'Awareness' = 'yes'
    'LawEnforcement' = 'yes'
    'LawComm' = 'no'
...
foreach ($question in $realisticResponses.Keys) {
    $response = $realisticResponses[$question]
...
Write-Host "`n=== SOUTH FLORIDA JSON CREATED ===" -ForegroundColor Green
...
Write-Host "This meets the South Florida requirement!"
