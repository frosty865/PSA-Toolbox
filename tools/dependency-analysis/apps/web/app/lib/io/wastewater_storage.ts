/**
 * Persist wastewater dependency answers and derived findings to localStorage.
 */
import type { DependencySessionSnapshot } from './sessionTypes';

const STORAGE_KEY = 'wastewater:storage';

const storage = typeof window !== 'undefined' ? localStorage : null;

export function loadWastewaterSession(): DependencySessionSnapshot | null {
...
export function saveWastewaterSession(snapshot: DependencySessionSnapshot): void {
  if (!storage) return;
  try {
    const toSave = {
      ...snapshot,
...
export function clearWastewaterSession(): void {
  if (!storage) return;
  try {
